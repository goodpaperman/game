...
{ 
	if( m_life<=0 && !CTankView::bShowDead )
	{
		return ; 
	}
	...
	//路线
	if( CTankView::bShowRoutine && m_state==move && m_life>0 )
	{
	...
	//攻击目标
	if( CTankView::bShowAttackDes && m_state==attack && m_life>0 && m_bPrepared)
	{
		pDC->MoveTo(m_routine.m_start.GetPoint()) ; 
		pDC->LineTo(m_routine.m_end.GetPoint()) ; 
...
//			TRACE("X : %d, Y : %d\n", x, y ) ; 
			...
			if( m_state==attack || m_state==defence )
			{
				...
				CTankObj *pTank = CBackground::m_tanks.GetAt(m_attackID) ; 
				if( pTank->m_state==defence && (pTank->m_type==enemy || CTankView::bAutoAttack) )
				{
...
void CTankObj::Defence(int id)
{
	...
	else if( CTankView::bAutoAttack && /*m_type==enemy &&*/ m_state==defence)	//enemy active stochastic
	{
		static int seed ; 
		srand((UINT)time(NULL)+seed) ;
		seed = rand() ; 
//		TRACE("SEED : %d\n" , seed) ; 
		if( seed%200==0 ) // 679 //1769
		{
			if( seed%2000!=0 ) // 191 //91
			{
		...
		switch( Flag )
		{
			...
			case 3 :
			{
					...
					&& pTank->m_life>0 )
				{
					...
					if( dis.Distance()<disMax  && pTank->m_life < nLife)
					{
						id = i ; 
						//disMax = dis.Distance() ; 
						nLife = pTank->m_life; 
