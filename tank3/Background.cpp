...
//	BOOL bAttack = uFlags & MK_CONTROL ; 
	SetCapture(); 
	BOOL bBatch = uFlags & MK_CONTROL; 
	CPointf pointf = CorrectCoordinate(CPointf(point)) ; 
	CPointf pos = pointf; 
	int nSpace = 2 * TANK_SIZE + 5; 
	...
	switch( nCreate )
	{
		case 0 :
			if(!bBatch)
			{
                pTank = new CTankObj(pointf, 0, CTankView::tankRank, 100+CTankView::tankFireRange) ; 
                m_tanks.Add(pTank) ; 
			}
			else 
			{
    			for(int i=0; i<4; i++)
    			{
					for(int j=0; j<4; j++)
					{
    					pos.x = pointf.x + i * nSpace;
						pos.y = pointf.y + j * nSpace; 
            			pTank = new CTankObj(pos, 0, CTankView::tankRank, 100+CTankView::tankFireRange) ; 
            			m_tanks.Add(pTank) ; 
					}
    			}
			}
		...
		case 1 :
			if(!bBatch)
			{
    			pTank = new CTankObj(pointf, 1, CTankView::tankRank, 100+CTankView::tankFireRange) ; 
				pTank->m_bodyAngle = pTank->m_gunAngle = PI; 
    			m_tanks.Add(pTank) ; 
			}
			else 
			{
				for(int i=0; i<4; i++)
				{
					for(int j=0; j<4; j++)
					{
    					pos.x = pointf.x + i * nSpace; 
						pos.y = pointf.y + j * nSpace; 
            			pTank = new CTankObj(pos, 1, CTankView::tankRank, 100+CTankView::tankFireRange) ; 
    					pTank->m_bodyAngle = pTank->m_gunAngle = PI; 
            			m_tanks.Add(pTank) ; 
					}
				}
...
BOOL CBackground::AvoidCrush(UINT i, UINT j)
{
	...
	double delta = (from->m_radius + to->m_radius - dis) ; 
